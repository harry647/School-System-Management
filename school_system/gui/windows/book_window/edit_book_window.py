...
Dedicated window for editing existing books.
"""
...
class EditBookWindow(BaseFunctionWindow):
    """Dedicated window for editing existing books."""
    
    book_updated = pyqtSignal()
    
    def __init__(self, book_id: str, parent=None, current_user: str = "", current_role: str = ""):
        """Initialize the edit book window."""
        super().__init__(f"Edit Book - {book_id}", parent, current_user, current_role)
    ...
    def _create_form_card(self) -> QWidget:
        ...
        save_btn = self.create_button("Update Book", "primary")
        save_btn.clicked.connect(self._on_update_book)
    ...
    def _load_book_data(self):
        ...
        try:
            ...
            show_error_message("Error", f"Failed to load book data: {str(e)}", self)
    ...
    def _on_update_book(self):
        ...
        if not title or not condition:
            show_error_message("Validation Error", "Please fill in all required fields (Title, Condition).", self)
            return
        ...
        try:
            ...
            self.book_service.update_book(self.book_id, book_data)
            show_success_message("Success", f"Book {self.book_id} updated successfully.", self)
            ...
            show_error_message("Error", f"Failed to update book: {str(e)}", self)
