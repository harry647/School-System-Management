...
class SchoolSystemApplication:
    ...
    def _show_login(self):
        ...
        try:
            self.login_window = LoginWindow(None, self._on_login_success)
            self.login_window.show()
            logger.info("Login window displayed")
        except Exception as e:
            logger.error(f"Failed to create login window: {e}")
            show_error_message("Error", f"Failed to display login window: {e}", None)
    ...
    def _on_login_success(self, username: str, role: str):
        ...
        try:
            self.main_window = MainWindow(None, username, role, self._on_logout)
            self.main_window.show()
            logger.info("Main application window created")
        except Exception as e:
            logger.error(f"Failed to create main window: {e}")
            show_error_message("Error", f"Failed to create main window: {e}", None)
...
Developed for efficient school administration.
    ...
    def _on_closing(self, event):
        ...
        current_window = self.main_window if self.main_window else self.login_window
    ...
    def show_message(self, title: str, message: str, message_type: str = "info"):
        ...
        if message_type == "error":
            show_error_message(title, message, current_window)
        elif message_type == "warning":
            from school_system.gui.dialogs.message_dialog import show_warning_message
            show_warning_message(title, message, current_window)
        else:
            show_info_message(title, message, current_window)
